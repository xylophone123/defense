...
\begin{frame}{Prefactorization Algebra Examples}
    For some associative algebra $A$ and $M \in A-\mbox{mod}$ we can define a prefactorization algebra $\mathcal{F}$ on $\R_{\geq 0}$. For open intervals $U,V,W \subseteq \R_{\geq 0}$ with $0\in U, W$
...
\begin{frame}{Attaching to Complex Curve}
    For a Riemann surface $X$ and $M$ a conformal $V$-module, FBZ construct the following sheaf
    \[
...
\begin{frame}{Derived Weyl Modules}
    In dimension 1 we can define a module for the Kac-Moody factorization algebra at $x \in \Sigma$. For $M\in \mathfrak{g}\mbox{-mod}$ we define
    \[
        \mathcal{M} =\Omega^{0,*}\otimes \mathcal{E}^\bullet \otimes M
    \]
    where $\mathcal{E}^\bullet$ is a resolution of the skyscraper sheaf at $x$. $\mathcal{M}$ is a dg-mod for $\mathfrak{g}^\Sigma $ in the obvious way. Then we can define the following precosheaf of dg-Lie Algebras
    \[
...
\begin{frame}{Raviolo Vertex Algebra}
    Raviolo vertex algebras were introduced by (GW). They are vertex algebras with multiplication parameterized by the formal raviolo
    \[
...
\begin{frame}{Raviolo Current Vertex Algebra}
    ...
    Let $\C_{\kappa} = \mbox{Sym}(\C\kappa[-1])$ be the representation of the Lie subalgebra $\hat{\mathfrak{g}}_{\geq 0} =  \C \kappa[-1] \oplus \mathfrak{g}[[u]]\subseteq \mathfrak{g}_h$ where $\kappa$ acts by multiplication by $\kappa$ and $\mathfrak{g}[[z]]$ acts by 0 .
...
\begin{frame}{Operad}
    The operad $B^3$ whose morphisms are given by configurations of balls. It is colored by the radius of the balls. Denote
    \[
        B_3(r_1,\dots,r_k|s)
    \]
...
\begin{tikzpicture}
    ...
    \draw (Z) circle (0.5);
    \fill (Z) circle (1.5pt);
    \draw (Z) -- ++(0.5,0) node[midway, below] {$r_k$};

    % Curved ellipsis made of three dots
    \fill (0.9,0.9) circle (0.8pt);
    \fill (1.25,0.82) circle (0.8pt);
    \fill (1.65,0.76) circle (0.8pt);

    % Big circle
    \draw (0,0) circle (3.5);
    \fill (0,0) circle (1.5pt);
    \draw (0,0) -- (3.5,0) node[midway, below] {$s$};
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{frame}{Cooperad}
    ...
    We can define the cooperad of smooth functions on $B^3$, $C^\infty(B^3)$, with pull back of the composition map. 
...
\begin{frame}{Locality}
    ...
    \]
    where we integrate $z$ over a larger sphere than $w$. Then from the following cycle relation
    \begin{tikzpicture}[every node/.style={font=\small}]
...
\begin{frame}{Raviolo Current Factorization Algebra}
    ...
    \[
        \mathcal{F} := \mathrm{Sym}(\hat{\mathfrak{g}}[1] ,d'+d_{CE}).
    \]
