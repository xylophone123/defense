...
\begin{frame}{Kac-Moody Factorization Algebra}
    The following is an example of a factorization envelope
    \[
        \mathcal{G}:= \Omega^{0,*}_c \otimes \mathfrak{g}
    \]
    For $h$ a symmetric bilinear form on $\mathfrak{g}$ define the central extension on $U$ as
    \[
        0 \to \C k \to \hat{\mathcal{G}} \to \mathcal{G} \to 0
    \]
    defined on $U$
    \[
        [\alpha \otimes X,\beta\otimes Y] = \alpha \wedge \beta \otimes [X,Y] - \int_{U} \alpha \partial \beta h(X,Y)
    \]
...
\begin{frame}{Kac-Moody Factorization Algebra}
    ...
    \begin{block}{Theorem}
        $U(1)$-invariant holomorphic prefactorization algebras with additional conditions produce vertex algebras on $\C$.
    \end{block}
    The Kac-Moody factorization algebra produces the Kac-Moody vertex algebra.
    \[
        V = \oplus_{k\in \Z}H^{*}(\mathcal{F}_{\mathfrak{g},k}(D)).
    \]
    The vertex operator comes from Laurent expanding the following diagram
\end{frame}
