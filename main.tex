...
\begin{frame}{Attaching to Complex Curve}
    A vertex algebra module is \textbf{quasi-conformal} if it has an action of $\mathrm{Der}(\mathcal{O})= \mathrm{Aut}(\C[[t]])$ which can be exponentiated to an action of $\mathrm{Aut}(\mathcal{O})$. For a Riemann surface $X$ and $M$ a quasi-conformal $V$-module, FBZ construct the following sheaf
    \[
...
\begin{frame}{Derived Weyl Modules}
    ...
    \]
    Taking the factorization envelope of compactly supported sections we have a prefactorization algebra
    \[
        \mathcal{F}_M = \mathrm{Sym}((\hat{\mathcal{G}}   \ltimes \mathcal{M})_c,d_{CE}+\bar{\partial}+d_{M}).
    \]
    Note at the level of cohomology for $U$ with $x \not\in U$
    \[
        \mathcal{F}_M (U) \cong \mathcal{F}_{\mathfrak{g}} (U)
    \]
\end{frame}
...
\begin{frame}{Coinvariants}
    ...
    \]
    We have an action of $\mathfrak{g}_{out} :=\mathfrak{g}[X\setminus \{x\}]$ on $V_k (\mathfrak{g})$ by taking residues. FBZ show that 
    \[
    H_{V}(X,(x_i),(M_i))_{i=1}^n =\frac{ \mathcal{M}_{1,x_1} \otimes \dots \otimes \mathcal{M}_{n,x_n} }{\mathfrak{g}_{out}\cdot ( \mathcal{M}_{1,x_1} \otimes \dots \otimes \mathcal{M}_{n,x_n})}.
    \] 
    If we construct $\mathcal{F}_M$ for $(M_i)$ inserted at $(x_i)$ on a compact Riemann surface $X$ then
    \begin{align*}
    H^0(\mathcal{F}^{(1,\dots,1)}(X)) \cong H_0(\Omega^{0,*}(X) \otimes \mathfrak{g} \oplus \C K , M_1\otimes \dots \otimes M_n) \\
     \cong H_0(R\Gamma(X,\mathcal{O}_X)\otimes \mathfrak{g} \oplus \C K,M_1\otimes \dots \otimes M_n ).
    \end{align*}
...
\begin{frame}{Coinvariants via Factorization Homology}
     \begin{block}{Theorem}
        ...
        \[
         H_0 ( \mathfrak{g} \otimes R\Gamma(X, \mathcal{O}_X), \C) \simeq H_0(\mathfrak{g}_{out}, W_1\otimes \dots \otimes W_n)
          \]
    \end{block}
...
\begin{frame}{Raviolo Current Vertex Algebra}
    ...
    Let $\C_{\kappa} = \mbox{Sym}(\C\kappa[-1])$ be the representation of the Lie subalgebra $\hat{\mathfrak{g}}_{\geq 0} =  \C \kappa[-1] \oplus \mathfrak{g}[[z]]\subseteq \mathfrak{g}_h$ where $\kappa$ acts by multiplication by $\kappa$ and $\mathfrak{g}[[z]]$ acts by 0 .
    ...
    \]
    For $\mu_{a,n}:= T_a \otimes \Omega^n_z$ and $J_{a,n}=T_a \otimes z^n$, for $(T_i)_{i=1}^n$ a basis of $\mathfrak{g}$, the generating vertex operators are
    \[
...
\begin{frame}{Operad}
    The operad $B^3$ has morphisms given by configurations of balls. It is colored by the radius of the balls. Denote
    \[
